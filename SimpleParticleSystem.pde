...
//  3. Change the color of the particles as a function of the bounce: implemented as color changing between blue and green each time the particle collides with the big sphere
...
void update(float dt){
  ...
  for (int i = 0; i < toGen; i++){
    ...
    bounceColor[numParticles] = false;
    numParticles += 1;
  }
  ...
  if ((leftPressed||rightPressed)&&(upPressed||downPressed))  obstacleVel.mul(1.0/sqrt(2));
...
void draw(){
  ...
  //fill(20,20,120);
  for (int i = 0; i < numParticles; i++){
    if(bounceColor[i])  fill(10,120,20);
    if(!bounceColor[i])  fill(20,20,120);
