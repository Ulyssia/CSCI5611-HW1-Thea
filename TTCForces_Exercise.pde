...
float computeTTC(Vec2 pos1, Vec2 vel1, float radius1, Vec2 pos2, Vec2 vel2, float radius2){
  float tot_rad = radius1+radius2;
  Vec2 rel_vel = vel1.minus(vel2);
  float ttc = rayCircleIntersectTime(pos2, tot_rad, pos2, rel_vel);
  return ttc;
}
...
Vec2 computeAgentForces(int id){
  ...
  //Vec2 goalVel = goalPos[id].minus(agentPos[id]).normalized();
  ...
  if(goalVel.length()<10)  return acc;
...
void moveAgent(float dt){
  ...
  for (int i = 0; i < numAgents; i++){
    ...
    if(agentPos[i].minus(goalPos[i]).length() < 0.7)  agentVel[i] = new Vec2(0,0);
